#!/usr/bin/env bash

#
# Desc: Apply a theme file from the themes directory by overwriting
#       the current theme file with it.
#
# Usage: apply_hyprlock_theme.sh "theme_name"
#

set -eo pipefail

if pacman -Q hyprlock &>/dev/null; then

  THEME_NAME="$1" # eg. tokyo_night

  HYPR_CONFIG_DIR="$HOME/.config/hypr"
  THEMES_DIR="$HYPR_CONFIG_DIR/themes/hyprlock"
  THEME_FILE="$THEMES_DIR/$THEME_NAME.conf"

  # Make sure theme name was provided
  if [[ -z $THEME_NAME ]]; then
    echo -e "\e[31m \e[0mHyprlock theme name argument wasn't provided."
    exit 1
  fi

  # Make sure the theme exists
  if [[ ! -f "$THEME_FILE" ]]; then
    echo -e "\e[31m \e[0mHyprlock theme '$THEME_NAME' not found... skipping..."
  else
    # Overwrite current theme file with a copy of the new theme's file.
    cp "$THEME_FILE" "$THEMES_DIR/current_theme.conf"

    echo -e "\e[32m✅ \e[0mHyprlock theme '$THEME_NAME' applied."
  fi
fi
